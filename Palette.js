define(["dojo/_base/lang", "dojo/_base/array", "./utils"],
	function(lang, arr, utils){
	...
	***************************************************************/
	...
	//	ctor ----------------------------------------------------------------------------
	var Palette = function(/* String|Array|dojox.color.Color|dojox.color.Palette */base){
		// summary:
		...
		if(base instanceof Palette){
			...
			this.colors = arr.map(base.slice(0), function(item){
				if(lang.isString(item)){ return new utils.Color(item); }
				return item;
			});
		}
		else if(lang.isString(base)){
			this.colors = [ null, null, new Color(base), null, null ];
		}
	}
	...
	function tRGBA(p, param, val){
		...
		arr.forEach(p.colors, function(item){
			...
			ret.colors.push(new utils.Color({
	...
	function tCMYK(p, param, val){
		...
		arr.forEach(p.colors, function(item){
			...
			ret.colors.push(utils.fromCmyk(
				Math.min(100, Math.max(0, c)),
				Math.min(100, Math.max(0, m)),
				Math.min(100, Math.max(0, y)),
	...
	function tHSL(p, param, val){
		...
		arr.forEach(p.colors, function(item){
			...
			ret.colors.push(utils.fromHsl(h%360, Math.min(100, Math.max(0, s)), Math.min(100, Math.max(0, l))));
		});
		return ret;
	}
	...
	function tHSV(p, param, val){
		...
		arr.forEach(p.colors, function(item){
			...
			ret.colors.push(utils.fromHsv(h%360, Math.min(100, Math.max(0, s)), Math.min(100, Math.max(0, v))));
		});
		return ret;
	}
...
=====*/
	...
	lang.extend(utils.Palette, {
		...
		clone: function(){
			...
			return new utils.Palette(this);	//	dojox.color.Palette
		}
	});

	lang.mixin(utils.Palette, {
		generators: {
			analogous:function(/* __analogousArgs */args){
				...
				var high=args.high||60, 	//	delta between base hue and highest hue (subtracted from base)
					...
					base = lang.isString(args.base)?new utils.Color(args.base):args.base,
					hsv=base.toHsv();
				...
				return new utils.Palette(arr.map(h, function(hue, i){
			...
			monochromatic: function(/* __generatorArgs */args){
				...
				//		http://kuler.adobe.com.
				var base = lang.isString(args.base)?new utils.Color(args.base):args.base,
					hsv = base.toHsv();
		...
		generate: function(/* String|dojox.color.Color */base, /* Function|String */type){
			...
			else if(utils.Palette.generators[type]){
				return utils.Palette.generators[type]({ base: base });	//	dojox.color.Palette
			}
